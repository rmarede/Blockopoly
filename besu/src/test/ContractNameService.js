...
describe("ContractNameService", function () {
    ...
    describe("Set", function () {
        ...
        it("Should return 0x0 for addresses not set", async function () {
            const { cns } = await loadFixture(deployCNSFixture);
            expect(await cns.getContractAddress("ADDRESS1")).to.equal("0x0000000000000000000000000000000000000000");
        });
    ...
    describe("Update", function () {
        it("Should update versions when set again", async function () {
            const { cns } = await loadFixture(deployCNSFixture);
