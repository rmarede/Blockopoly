...
describe("RentalAgreement", function () {
    ...
    describe("Enroll", function () {
        ...
        it("Should not enroll if transfer fails", async function () {
            const { rentalAgreement, wallet } = await loadFixture(deployRentalAgreementFixture);
            const [acc1, acc2] = await ethers.getSigners();
            
            await expect(wallet.connect(acc2).setTransferFrom(false)).not.to.be.reverted;
    ...
    describe("Return Deposit", function () {
        ...
        it("Should apply penalty", async function () {
            ...
            await expect(wallet.setBalanceOf(10)).not.to.be.reverted;
