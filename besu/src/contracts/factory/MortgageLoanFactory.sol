...
contract MortgageLoanFactory is Context {
    ...
    function createMortgageLoan(MortgageLoan.LoanDetails memory _details) public returns (address) { 
        ...
        if (roleRegistry != address(0)) {
            require(IRoleRegistry(roleRegistry).canMintLoans(IAccountRegistry(accountRegistryAddress()).roleOf(msg.sender)), "MortgageLoanFactory: sender does not have permission to mint");
        }
